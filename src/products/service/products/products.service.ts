...
export class ProductsService { /* All the required repositories and services are injected in the constructor
    ...
    */
    async updateProductStock(productid : number, updatedStockPercentage : number, updatedStock : number, mailSentFlag : number): Promise < number > {
        try {
                ...
                if (updatedStockPercentage < 20 && mailSentFlag != 1) {
                    ...
                    if (mailStatus == 1) { // If mail was sent successfully
                        await this.productRepository.update({
                            productId: productid
                        }, {mailSentFlag: 1}); // Update the mail sent Flag to avoid the sending mails again in future
                    }
                }
                return 1
 ...
 */
    async sendMail(productid : number, subject : string, templateName : string): Promise < number > {
        // Using mailerService to send the mail
